...
For [zjherder](../zjherder) bar widgets, render with its markup instead —
...
No background tokens: zjherder's bar renders on the plain terminal
background (an ANSI-8 bg reads as a dim box, not surface0).

Wire it as the host's `[bar.widget.stats]` in `~/.config/zjherder/config.toml`
(see zjherder's `docs/config.md` § Cross-machine widgets). The same config —
